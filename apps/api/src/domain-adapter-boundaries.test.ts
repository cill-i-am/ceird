...
describe("api domain adapter boundaries", () => {
  it("keeps product domain implementations out of the public API app", async () => {
    const files = await listSourceFiles(apiSrcDir);
    const relativeFiles = files.map((file) => path.relative(apiSrcDir, file));
    ...
    expect(relativeFiles).not.toContain("domains/jobs/activity-recorder.ts");
    ...
    expect(relativeFiles).not.toContain(
      "domains/organizations/authorization.ts"
    );
  });
