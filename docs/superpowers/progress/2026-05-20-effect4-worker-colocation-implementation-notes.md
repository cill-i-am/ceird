...
- During simplify I left shared Cloudflare compatibility/observability literals and MCP tool metadata duplication alone because they were outside that pass's cache/dependency scope. Those were later closed in the structural follow-up pass by introducing `infra/cloudflare-worker-defaults.ts` and canonical MCP tool definitions.
...
- 2026-05-20: Closed the requested structural follow-ups. Cloudflare Worker compatibility and observability now live in `infra/cloudflare-worker-defaults.ts` and are shared by the app-owned Worker declarations. MCP tool definitions now provide the canonical name/scope/admin/description/read-only/destructive metadata used by registration, tool construction, and authorization. Auth email and auth email queue errors now use reverse-domain `_tag` constants, with `catchTag` call sites updated to those constants. `JobsService` no longer uses the generic mutation-failure helper; each transaction boundary catches the specific repository failure tags it can emit, and actor-membership loss is handled by small membership-specific helpers without local casts.
