...
Worker runtime env vars. When the auth token, org, or project value is missing,
`apps/app/vite.config.ts` disables browser source-map uploads while leaving
runtime Sentry instrumentation enabled.
