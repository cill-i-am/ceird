...
| [Captcha](https://better-auth.com/docs/plugins/captcha)                           | Security/abuse             | Adopted via Ceird plugin                     | Cloudflare Turnstile protects sign-up, password reset request, and verification resend when enabled. Ceird owns Siteverify timeout and provider-failure telemetry; conditional failed-sign-in challenges remain deferred to `TSK-116`. |
...
3. Add captcha selectively.
   ...
   - Provider operations: Ceird owns Turnstile Siteverify with a 3000 ms default
     timeout, fail-closed behavior, and sanitized provider-failure telemetry.
