...
| Email verification                    | Partially adopted           | Verification emails are sent and high-trust org/OAuth actions are gated.                                                                                                     |
...
| Session management                    | Adopted for account control | Session lookup, sign-out, active-session listing, current-session marking, targeted other-session revocation, and revoke-all-other-sessions are exposed in account settings. |
...
| Built-in rate limiter                 | Adopted                     | Database-backed with custom rules, observable storage, and pre-handler abuse reservations for sensitive routes, including actor/org invitation caps.                         |
...
| Versioned secrets                     | Adopted                     | `BETTER_AUTH_SECRETS` feeds Better Auth `secrets` sorted by highest version while fallback stays configured.                                                                 |
| Hooks and schema customization        | Adopted                     | Organization validation and email delivery use hooks.                                                                                                                        |
| Organization/access control           | Adopted with custom policy  | Better Auth owns membership, normal-path structural limits, and endpoint permissions; Ceird owns domain authorization.                                                       |
...
| --------------------------------------------------------------------------------- | -------------------------- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [Two-Factor Authentication](https://better-auth.com/docs/plugins/2fa)             | Authentication/security    | Adopted for first-release TOTP   | Optional TOTP plus backup codes are wired through Better Auth with settings management and inline login challenge UX. Trusted-device policy and sensitive-action step-up auth remain follow-up work.                                  |
...
| [Agent Auth](https://better-auth.com/docs/plugins/agent-auth)                     | API/agents                 | Not adopted                      | Deferred by `TSK-69`; relevant to future public agent-capability workflows, but docs mark it unstable and adoption needs a concrete pilot.                                                                                            |
...
| [MCP](https://better-auth.com/docs/plugins/mcp)                                   | OAuth/MCP                  | Not adopted as standalone plugin | Rejected by `TSK-69`; Better Auth docs say this plugin will soon be deprecated in favor of OAuth Provider, and Ceird already uses OAuth Provider `mcpHandler`.                                                                        |
...
| [Captcha](https://better-auth.com/docs/plugins/captcha)                           | Security/abuse             | Partially adopted                | Cloudflare Turnstile protects sign-up, password reset request, and verification resend when enabled. Conditional failed-sign-in challenges are deferred to `TSK-116`; provider timeout and telemetry policy is deferred to `TSK-121`. |
| [Have I Been Pwned](https://better-auth.com/docs/plugins/have-i-been-pwned)       | Security/passwords         | Adopted via Ceird plugin         | Ceird uses the HIBP k-anonymity range API with fail-open telemetry to match approved outage policy.                                                                                                                                   |
...
1. Add versioned Better Auth secret support.
   - Status: baseline implemented; stage rollout and an operational rotation
     runbook remain.
   ...
   - Implementation shape: parse a `BETTER_AUTH_SECRETS` style config value,
     sort the highest version first before passing Better Auth `secrets`,
     document rotation in `auth.md`, and keep the existing single secret as a
     migration fallback.

2. Add Have I Been Pwned on top of the explicit password policy.
   - Status: adopted through Ceird's Better Auth password compromise check
     plugin because the stock plugin did not provide the approved fail-open
     outage behavior.
   - Security value: blocks known-compromised passwords at sign-up, reset, and
     change-password boundaries.
...
4. Harden OAuth/MCP registration and consent policy.
   ...
   - Scope: constrain write/admin scope registration, improve consent copy,
     and audit registrations/grants. `TSK-69` keeps the current OAuth Provider
     MCP path; Device Authorization remains a separate CLI/limited-input
     evaluation.

5. Define verified-email gates.
   - Status: baseline implemented for organization creation, member
     invitations, and Ceird-scope OAuth/MCP consent approval.
...
1. Continue hardening two-factor authentication for privileged users.
   - Backend plugin/schema support now exists for TOTP and backup codes.
   - Account settings now expose optional TOTP enrollment, required backup-code
     acknowledgement, backup-code regeneration, and disable controls.
   - Strongly prompt 2FA for owners/admins; hard requirements remain deferred.
   - Login now handles Better Auth's two-factor sign-in redirect inline with
...
3. Add captcha selectively.
   - Status: adopted for sign-up, password reset request, and verification
     resend through Cloudflare Turnstile.
   - Remaining gap: conditional captcha after repeated failed sign-in attempts
     is a backlog spike in `TSK-116`; ordinary sign-in remains captcha-free
     until that design exists.
   - Remaining operations gap: captcha provider timeout and sanitized telemetry
     policy is a backlog spike in `TSK-121`.
...
3. Revisit Agent Auth only after stability and product triggers are met.
   ...
   - Treat adoption as a product-led public agent-capability pilot, not as a
     drop-in replacement for the current MCP OAuth Provider path.
