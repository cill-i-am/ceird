...
| Organization security activity         | JSONB metadata search, event-scoped target joins, microsecond cursor formatting, and cursor predicates                                                        | The audit log list is JSONB and cursor heavy. Raw SQL keeps target search and stable cursor formatting visible in one query. Organization audit writes, selected read rows, event-specific metadata, and cursor state decode through the identity persistence schemas before service or HTTP mapping. |
