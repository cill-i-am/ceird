...
| Comments repository                    | comment list reads plus CTE ownership writes for work-item and site comments                                                                                  | Comment ownership rows and inserted comments are coupled through CTE projections, site comment creation locks the active site, and Sites comments also maintain the product-safe `site_comment_bodies` Electric projection. Migrate only as a whole comments repository slice with focused comment tests. |
