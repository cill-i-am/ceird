...
Raw SQL rows should still become trustworthy at the repository boundary. The
connected-app grants repository is the current auth proof slice: it keeps raw
SQL for lateral token counts and the audited disconnect transaction, but decodes
list rows, disconnect consent rows, and `oauth_consent_revoked` audit write
values through Effect Schema before returning values to services.
...
| Connected-app listing and disconnect   | lateral token-count projections decoded at the repository boundary, transactional consent delete, refresh-token revoke, access-token delete, and decoded JSONB audit insert values | The path is Better Auth adjacent and security-sensitive. The list query depends on lateral aggregates, and disconnect must keep revocation plus audit as one transaction while preventing raw SQL projections from escaping the repository.                                           |
