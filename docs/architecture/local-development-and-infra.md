...
| `CEIRD_ELECTRIC_STORAGE_ACCESS_KEY_ID`              | Electric R2 S3 access key consumed by production/full-sync app-stack deploys.                                    |
| `CEIRD_ELECTRIC_STORAGE_SECRET_ACCESS_KEY`          | Electric R2 S3 secret key consumed by production/full-sync app-stack deploys.                                    |
...
and provisions the stage R2 bucket used by deployed Electric stages. Local
Alchemy dev stages also create a stage-scoped R2 API token so the storage path
is ready when container-backed local parity becomes available. The stack does
not create or attach the Cloudflare Container application in local Alchemy dev:
the Containers API validates against a cloud Durable Object namespace, while
...
of pretending sync is active. Deployed stages consume the
...
management. The credential stack writes those Electric runtime secrets to the
`main` GitHub environment only, and scopes the token to the production Electric
R2 bucket. Pull-request previews and push-to-main cloud E2E stages intentionally
do not receive production Electric storage credentials; they reconcile the sync
Worker and authorization path, then use a shallow unauthorized-shape probe until
a per-stage runtime credential flow is added. The app stack passes configured
credential values, the Neon branch URL, and the generated Electric source secret
into the Sync Worker as secrets; the `ElectricSql` Durable Object supplies them
to the Cloudflare Container as startup environment variables when it starts
Electric. Production and ordinary non-preview deployed stages fail closed when
...
and Neon resources with stage-scoped names, but Electric container execution is
deploy-only for now.
...
that the container mounts at `/var/lib/electric` before Electric starts. Local
dev stages mint a bucket-scoped token in the app stack but deliberately skip
the Container application because Cloudflare can only attach Containers to a
cloud Durable Object namespace. CI and deployed stages consume Electric R2 S3
credentials from the GitHub credential stack. The root stack also derives the
`ElectricSql` Durable Object placement hint from the Neon region. The
