...
storage mount. There is no separate local Docker service in the default
workflow; local cloud-backed stages exercise the same Worker, Durable Object,
Container, R2, and Neon resources as production with stage-scoped names.
...
- native Alchemy Cloudflare R2 bucket and bucket-scoped API token for Electric
  shape storage
...
runs Electric SQL. The root stack declares the stage R2 bucket and token that
the container uses to mount `/var/lib/electric` before Electric starts, and it
