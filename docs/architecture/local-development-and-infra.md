...
| `CEIRD_ELECTRIC_STORAGE_ACCESS_KEY_ID`     | Electric R2 S3 access key consumed by production app-stack deploys.                               |
| `CEIRD_ELECTRIC_STORAGE_SECRET_ACCESS_KEY` | Electric R2 S3 secret key consumed by production app-stack deploys.                               |
...
for the container. Production deploys consume the
...
management. Pull-request previews and push-to-main cloud E2E stages deliberately
do not receive the shared production Electric storage secrets; they publish the
sync Worker but skip the Electric Container and R2 bucket until a future
stage-scoped credential path is added. Production and other non-preview deployed
stages still fail closed when Electric storage secrets are absent.
