...
| `AUTH_CAPTCHA_SITE_VERIFY_REQUEST_TIMEOUT_MS`       | Optional Turnstile Siteverify request timeout; defaults to 3000 ms in domain config.                             |
...
| `AUTH_CAPTCHA_TURNSTILE_SECRET_KEY`                 | Domain-only Turnstile secret key used by Ceird's Better Auth captcha plugin.                                     |
...
| `AUTH_CAPTCHA_SITE_VERIFY_REQUEST_TIMEOUT_MS`       | `3000`          | Domain Worker timeout budget for each Turnstile Siteverify request. Provider timeout fails closed and emits sanitized captcha-provider telemetry.                                             |
...
| `AUTH_CAPTCHA_TURNSTILE_SECRET_KEY`                 | unset           | Redacted Turnstile secret passed only to the domain Worker for Ceird-owned captcha verification.                                                                                              |
