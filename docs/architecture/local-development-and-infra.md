...
for the container. CI and deployed stages instead consume the
...
management. Pull-request previews and push-to-main cloud E2E stages have one
bootstrap exception: until the main credential stack has minted those Electric
R2 secrets and updated the CI deploy token with R2 and Container permissions,
ephemeral deploys without both Electric storage secrets publish the sync Worker
but skip the Electric Container and R2 bucket. Production and other non-preview
deployed stages still fail closed when the Electric storage secrets are absent.
...
the private domain Worker before starting Playwright. Sync health can be probed
at the stage sync origin when sync-dependent tests are added. This avoids
transient route, domain, TLS, or service binding propagation failures on freshly
created preview hostnames. The domain Worker
