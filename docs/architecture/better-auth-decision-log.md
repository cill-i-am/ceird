...
| Captcha provider timeout and telemetry       | `TSK-121`                     | Approved | Own Turnstile Siteverify in Ceird with a 3000 ms default timeout, fail closed for selected public abuse endpoints, reuse existing captcha failure UX, and emit sanitized high-severity `captcha_provider_failure` telemetry for sustained alerting.    |
...
| OAuth DCR default refresh-token grant policy | `TSK-130`                     | Approved | Keep Better Auth's default: omitted public DCR `grant_types` persist as `authorization_code` only. Refresh-token metadata requires both supported grants; runtime token issuance remains Better Auth-native.                                           |
