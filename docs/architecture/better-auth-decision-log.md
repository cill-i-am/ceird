...
| OAuth DCR default refresh-token grant policy | `TSK-130`                     | Deferred | Decide whether omitted public DCR `grant_types` should stay Better Auth-native (`authorization_code` only) or default to refresh support when Ceird defaults scopes to include `offline_access`.                                                       |
