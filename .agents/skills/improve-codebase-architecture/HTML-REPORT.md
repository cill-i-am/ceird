...
terms from `/codebase-design` without ceremony.
...
from `/codebase-design`. Concision is not an excuse to drift.
...
term isn't in `/codebase-design`, reach for one that is before inventing a new
one.
